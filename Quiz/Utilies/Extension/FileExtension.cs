namespace Quiz.Utilies.Extension
{
    public static class FileExtension
    {
        ...
        public static bool CheckValidate(this IFormFile file, string type, int kb)
        {
            string result = "";
            if (!file.CheckSize(kb))
            {
                result += $"Faylinn hecmi {kb} kb-dan artiq olmamalidir.";
            }
            if (file.CheckType(type))
            {
                result += $"Faylinn tipi yalnisdir.";
            }
            return false;
        }
